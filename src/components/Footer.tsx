...
export default function Footer({ footerData, restaurantName, logoUrl, slug, forceMobile = false }: FooterProps) {
  ...
  return (
    <footer className="bg-[var(--tenant-footer-bg,var(--tenant-background,#FFF8E7))] border-t-2 border-[var(--tenant-secondary,#D4AF37)] mt-16">
      <div className="container mx-auto px-4 py-12 max-w-7xl">
        ...
        <div className={`grid grid-cols-2 gap-8 mb-8 ${forceMobile ? '' : 'md:grid-cols-3'}`}>
          ...
          {showBrandColumn && (
            <div className={`col-span-2 text-center ${forceMobile ? '' : 'md:col-span-1 md:text-left'}`}>
              ...
              />
              <p className="text-sm text-[var(--tenant-footer-text,var(--tenant-text-secondary,#4B5563))] leading-relaxed mb-4">
          ...
          <div className={`text-center ${forceMobile ? '' : 'md:text-left'}`}>
            <h4 className="text-lg font-bold text-[var(--tenant-footer-text,var(--tenant-text,#171717))] mb-4">
              {language === 'it' ? 'Le Nostre Sedi' : 'Our Locations'}
            </h4>
            {locations.length > 0 && (
              <ul className="space-y-3 text-sm text-[var(--tenant-footer-text,var(--tenant-text-secondary,#4B5563))] mb-4">
          ...
          <div className={`text-center ${forceMobile ? '' : 'md:text-left'}`}>
            ...
            {/* Social Media */}
            <div>
              <h5 className="text-sm font-semibold text-[var(--tenant-footer-text,var(--tenant-text,#171717))] mb-3">
                  ...
                  socials.map((social, index) => {
                    ...
                    return (
                      <a
                        ...
                        className="text-[var(--tenant-footer-text,var(--tenant-text-secondary,#4B5563))] hover:text-[var(--tenant-primary,#8B0000)] transition-colors hover:scale-110 transform duration-200"
        ...
        <div className="text-center space-y-2">
          <p className="text-xs text-[var(--tenant-footer-text,var(--tenant-text-secondary,#4B5563))]">
